...
def create_2d_descriptors(smiles: str) -> Union[npt.NDArray[np.float_], List[float]]:
    ...
    use_descList = list(filter(lambda p: not p[0] in ["AvgIpc", "SPS"], Descriptors.descList))
...
def predict_logpapp(csvfile: str) -> Union[npt.NDArray[np.float_], pd.Series, List[float]]:
    ...
    descriptions_df = pd.DataFrame(data=descriptions,columns=[i for i, _ in use_descList])

    X = descriptions_df.values
    y = df['LogP app'].values
    X_train, X_test, y_train, _ = train_test_split(X, y, train_size=700, random_state=0)
...
def grid_search(csvfile: str) -> float:
    ...
    np.random.seed(0)
    tuned_parameters = [{'n_estimators': [100, 200, 400], 'max_depth': [5, 10, 15]}]
    clf = GridSearchCV(
